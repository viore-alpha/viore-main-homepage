...
test("server-renders the Korean Company story as the homepage", async () => {
  ...
  assert.match(html, /class="company-energy-canvas company-energy-layer-0"/);
...
test("server-renders the Alphadoc product story from real product UI", async () => {
  ...
  assert.match(productSource, /<CompanyEnergyCanvas quality="balanced" motion="ambient" \/>/);
...
test("carries the hero energy-line language into a slow scroll-linked convergence", async () => {
  ...
  assert.match(energyCanvas, /const ENERGY_LAYER_COUNT = 2/);
  assert.match(energyCanvas, /field\.classList\.toggle\("is-motion-active"/);
  assert.match(energyCanvas, /const releaseCanvases = \(\) =>/);
  assert.match(energyCanvas, /const layerCount = motion === "ambient" \? 1 : ENERGY_LAYER_COUNT/);
  assert.match(energyCanvas, /Array\.from\(\{ length: layerCount \}/);
  ...
  assert.match(companyBackdrop, /const NETWORK_FRAME_COUNT = 5/);
  assert.match(companyBackdrop, /className="company-convergence-canvas"/);
  assert.doesNotMatch(companyBackdrop, /company-convergence-canvas-start|company-convergence-canvas-end/);
  ...
  assert.match(companyBackdrop, /const syncVisibleFrame = \(\) =>/);
  assert.match(companyBackdrop, /document\.createElement\("canvas"\)/);
  ...
  assert.match(companyBackdrop, /const smoothstep =/);
  assert.match(companyBackdrop, /const convergence = smoothstep\(localProgress\)/);
  assert.match(companyBackdrop, /lateralScale = compact \? 0\.72 : 1/);
  ...
  assert.match(companyBackdrop, /motionPhase \+ index \* 0\.13 \+ family \* 1\.7/);
  assert.match(companyBackdrop, /context\.lineWidth = haze \? 16 : accent \? 2\.35 : 0\.76/);
  ...
  assert.doesNotMatch(css, /company-convergence-canvas-start|company-convergence-canvas-end/);
  assert.doesNotMatch(css, /company-network-composite-drift|scaleX\(calc\(1 - var\(--company-convergence-progress\)/);
  assert.doesNotMatch(css, /(?:^|\n)\.company-convergence-canvas \{[^}]*will-change:/);
  assert.doesNotMatch(css, /\.company-network-backdrop\.is-motion-active \.company-convergence-canvas \{[^}]*will-change:/);
  assert.match(css, /\.company-energy-field\.is-motion-active \.company-energy-layer-0 \{ animation: company-energy-thread-flow-a/);
  assert.match(css, /\.company-energy-field\.is-motion-active \.company-energy-layer-1 \{ animation: company-energy-thread-flow-b/);
  assert.match(css, /@keyframes company-energy-thread-flow-b/);
